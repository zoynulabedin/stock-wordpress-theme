...
if ( ! function_exists( 'stock_crazycafe_posted_on' ) ) :
	...
	function stock_crazycafe_posted_on() {
		...
		$display_post_by_date = cs_get_option('display_post_by_date');
		$posted_on = sprintf(
...
if ( ! function_exists( 'stock_crazycafe_entry_footer' ) ) :
	...
	function stock_crazycafe_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			$display_post_in_category = cs_get_option('display_post_in_category');

			if($display_post_in_category == true){
				/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'stock-crazycafe' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'stock-crazycafe' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			}
			
			$display_post_tags = cs_get_option('display_post_tags');
			if($display_post_tags==true){
				/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'stock-crazycafe' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'stock-crazycafe' ) . '</span>', $tags_list ); // WPCS: XSS OK.
				}
			}
